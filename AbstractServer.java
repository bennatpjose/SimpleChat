...
// INSTANCE METHODS *************************************************
   ...
   */
  public void sendToAllClients(Object msg) // IMPORTANT
  {
    Thread[] clientThreadList = getClientConnections();

    for (int i=0; i<clientThreadList.length; i++)
    {
      System.out.println("DEBUG: Sending to " + ((ConnectionToClient)clientThreadList[i]).getInfo("userName")+((ConnectionToClient)clientThreadList[i]).getInetAddress());
      try
      {
